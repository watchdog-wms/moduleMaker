...
if [ "${JFX_SDK_LIB_PATH}" == "" ]; then
	RET=$(find / -name "${JAVA_FX_TEST_NAME}")
	CODE=$?
	if [ ${CODE} -eq 0 ]; then
		C=1
		SEL=()
...
java -Djdk.gtk.version=2 --add-opens javafx.fxml/javafx.fxml=ALL-UNNAMED --add-opens javafx.controls/com.sun.javafx.scene.control=ALL-UNNAMED --module-path "${JFX_SDK_LIB_PATH}" --add-modules ${REQUIRED_JFX_MODULES} -jar "${SCRIPT_FOLDER}/moduleMaker.jar" $@
exit $?
